...
df_total_gdp_wider  = df_total %>% 
  select(Country.name, year, Logged.GDP.per.capita) %>%  
  pivot_wider(names_from = year, values_from = Logged.GDP.per.capita)

df_total_ss_wider  = df_total %>% 
  select(Country.name, year, Social.support) %>%  
  pivot_wider(names_from = year, values_from = Social.support)

df_total_le_wider  = df_total %>% 
  select(Country.name, year, Healthy.life.expectancy) %>%  
  pivot_wider(names_from = year, values_from = Healthy.life.expectancy)

df_total_freedom_wider  = df_total %>% 
  select(Country.name, year, Freedom.to.make.life.choices) %>%  
  pivot_wider(names_from = year, values_from = Freedom.to.make.life.choices)

df_total_generosity_wider  = df_total %>%
  select(Country.name, year, Generosity) %>%  
  pivot_wider(names_from = year, values_from = Generosity)

df_total_corruption_wider  = df_total %>%
  select(Country.name, year, Perceptions.of.corruption) %>%  
  pivot_wider(names_from = year, values_from = Perceptions.of.corruption)
...
```{r datavisualization_increasedhappiness, echo=FALSE, fig.width=12, fig.height=6}
